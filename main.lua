...
local executeURL = rootURL.."_/TranslateWebserverUi/data/batchexecute"
local fsid, bl
...
properties.Text = "[Translator]: Say > followed by an alpha code to switch to that language (e.g. >en for english) or >d to disable translation."
...
function disableSend()
    sendEnabled = false
    properties.Text = "[Translator]: Disabled translation."
    StarterGui:SetCore("ChatMakeSystemMessage", properties)
end
                ...
                if Enter ~= false and Bar['Text'] ~= '' then
                    ...
                    elseif Message:sub(1,1) == ">" and not Message:find(" ") then
                        if getISOCode(Message:sub(2)) then
                            ...
                            properties.Text = "[Translator]: Invalid alpha code."
                            StarterGui:SetCore("ChatMakeSystemMessage", properties)
                        end
